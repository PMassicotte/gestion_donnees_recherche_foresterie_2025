...
subtitle: "Making your data projects more reproducible, reusable and open"
...
format:
  revealjs:
    ...
    footer: "[{{< fa house >}} https://pmassicotte.github.io/gestion_donnees_recherche_foresterie_2026](https://pmassicotte.github.io/gestion_donnees_recherche_foresterie_2026)"
    height: 1080
    width: 1920
    slide-number: c/t
    transition: fade
...
citation:
  type: speech
  url: https://pmassicotte.github.io/gestion_donnees_recherche_foresterie_2026
  language: fr-CA
---
...
![](./img/logo/terminal_phil.png){fig-alt="Badge showing the name of the presenter (Philippe Massicotte)."}
:::

::: {.column width=50% .spaced}

{{< fa brands github >}} https://github.com/PMassicotte

{{< fa envelope >}} philippe.massicotte@takuvik.ulaval.ca

{{< fa brands x-twitter >}} @philmassicotte
...
Unless otherwise stated, all images in this presentation are AI generated.
...
3. **Human-readable**: [or have open source tools available for reading and writing]{.emphasize}.
...
::: aside
Data source: Horst AM, Hill AP, Gorman KB (2020). palmerpenguins: Palmer Archipelago (Antarctica) penguin data. R package version 0.1.0. [https://allisonhorst.github.io/palmerpenguins/](https://allisonhorst.github.io/palmerpenguins/){target="\_blank"} Artwork by @allison_horst.
:::

## Common open-source geographic file formats

These files contain information on geographic features such as [points, lines or polygons]{.emphasize}. There are a ton of [geographical file formats](https://gisgeography.com/gis-formats/){target="\_blank"}, but here are some that are particularly popular.
...
- GeoParquet (`.parquet`) is an [Open Geospatial Consortium (OGC) standard](https://geoparquet.org/){target="\_blank"} standard that adds interoperable geospatial types (Point, Line, Polygon) to [Apache Parquet](https://parquet.apache.org/){target="\_blank"}.
...
#| crop: true
#| dev: ragg_png
#| fig-alt: "A triangle polygon plotted with ggplot2, defined by three GeoJSON coordinates shown as red points at each vertex."
...
Create your own GeoJSON file online at [https://geojson.io/](https://geojson.io/){target="\_blank"}
...
>
> [Wikipedia](https://en.wikipedia.org/wiki/GeoTIFF){target="\_blank"}

::: {.center-align}

![](img/ai/geotiff.png){height=600 fig-alt="AI generated image of a computer screen showing some sci-fi images."}
...
```{r}
#| label: sst-pixel-values
#| fig-height: 3
#| fig-format: png
#| crop: true
#| fig-dpi: 300
#| fig-alt: "Close-up view of Sea Surface Temperature pixel values in the St-Lawrence River, with numerical temperature values labeled within each raster cell."
pixel_values <- r[poly] |>
  as_tibble() |>
  rename(sst = 3L)
...
# Data project good practices {background-color="#2C404A"}

::: {.center-align}

![](./img/ai/working_with_other_data.png){height=500 fig-alt="AI generated image of a scientist looking at a computer screen with data on it."}
...
- Do not make any changes or corrections to the original raw data files.
  - [Changes made today may turn out to be wrong tomorrow.]{.emphasize}
...
[Source: [Preserve information: Keep raw data raw](https://dataoneorg.github.io/Education/bestpractices/preserve-information-keep){target="\_blank"}]{.small}
...
- Choosing a logical and consistent way to organize your data files makes it easier for you and your colleagues to find and use your data.
  - [Each project should be self-contained and include all the necessary files to reproduce the analyses and results.]{.emphasize}
...
- Good documentation helps making your research [reproducible]{.emphasize} and [reusable]{.emphasize}.

- Data without documentation [loses its meaning over time]{.emphasize}.

- A common scenario:
  - a student finishes their degree and [nobody can understand their data or scripts]{.emphasize}.
...
```{=html}
<div style="border: 1px solid #d0d7de; border-radius: 6px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; font-size: 22px; width: 100%; height: 80vh; display: flex; flex-direction: column; box-sizing: border-box; background-color: #ffffff;">
  <div style="background: #f6f8fa; padding: 12px 24px; border-bottom: 1px solid #d0d7de; border-radius: 6px 6px 0 0; color: #57606a; display: flex; align-items: center; gap: 8px; flex-shrink: 0;">
    📄 <strong>README.md</strong>
  </div>
  <div style="padding: 24px 40px; line-height: 1.6; flex: 1; display: flex; flex-direction: column; justify-content: space-evenly;">
    <h1 style="font-size: 1.4em; border-bottom: 1px solid #d0d7de; padding-bottom: 6px; margin: 0;">Forest Inventory 2024</h1>
    <div>
      <h2 style="font-size: 1.1em; margin: 0 0 4px 0;">Project description</h2>
      <p style="margin: 0;">Inventory of tree species and diameter at breast height (DBH) in the Montmorency Forest, collected in summer 2024.</p>
    </div>
    <div>
      <h2 style="font-size: 1.1em; margin: 0 0 4px 0;">Directory structure</h2>
      <pre style="background: #f6f8fa; padding: 12px 18px; border-radius: 6px; font-size: 18px; line-height: 1.6; margin: 0;">project/
├── data/
...
└── README.md</pre>
    ...
    <div>
      <h2 style="font-size: 1.1em; margin: 0 0 4px 0;">How to run</h2>
      <p style="margin: 0;">Run scripts in order: <code style="background:#f6f8fa; padding: 2px 6px; border-radius: 4px;">01_clean.R</code> → <code style="background:#f6f8fa; padding: 2px 6px; border-radius: 4px;">02_analysis.R</code> → <code style="background:#f6f8fa; padding: 2px 6px; border-radius: 4px;">03_figures.R</code></p>
    </div>
    <div>
      <h2 style="font-size: 1.1em; margin: 0 0 4px 0;">Contact</h2>
      <p style="margin: 0;">Jane Doe — <a href="mailto:jane.doe@ulaval.ca" style="color: #0969da;">jane.doe@ulaval.ca</a></p>
    </div>
  </div>
</div>
```
...
- Avoid commenting the [what]{.emphasize}, and instead comment the [why]{.emphasize}.
...
Bad [{{< fa times >}}]{style="color: #E6352FFF;"}
...
Good [{{< fa check >}}]{style="color: #34A74BFF;"}
...
# File and variable naming {background-color="#2C404A"}
...
![Source: [https://bit.ly/2M8cViI](https://bit.ly/2M8cViI){target="_blank"}](img/screenshots/python_file_naming.png){fig-alt="Screenshot of a scientific taking his head in his hands and sitting at a desk."}
...
[How to name files - Jennifer Bryan (YouTube)](https://www.youtube.com/watch?v=ES1LTlnpLMk){target="\_blank"}

[How to name files - Jennifer Bryan (Slides)](https://speakerdeck.com/jennybc/how-to-name-files-the-sequel){target="\_blank"}
...
![](img/ai/tidydata.png){height=800 fig-alt="AI generated cartoon comparing messy data versus tidy data. On the left, a frustrated scientist surrounded by chaotic papers labeled 'A total nightmare'. On the right, a happy scientist with an organized spreadsheet labeled 'Pure bliss' with checkmarks for clean, organized, and easy to analyze."}
...
- Many researchers structure their data in such a way that it is easily manipulated by a human, [but not so much programmatically]{.emphasize}.
...
After proper transformations, the data is now tidy ([or in normal form](https://en.wikipedia.org/wiki/Database_normalization){target="\_blank"}). [Each column is a variable, and each row is an observation.]{.emphasize}
...
![](img/ai/spreadsheet.png){height=800 fig-alt="AI generated image of a piece of paper showing a spreadsheet."}
...
![](img/ai/hd_fail.png){height=800 fig-alt="AI generated image of a hard drive failing with a person crying because of losing data."}
...
[^1]: [Network World](https://www.networkworld.com/article/3285652/backup-vs-archive-why-its-important-to-know-the-difference.html){target="\_blank"}

[^2]: [Dorey, J & Nappert, M. (2024, 7 novembre). La gestion des données de recherche : briser les silos et bâtir l’avenir [conférence]. Congrès des professionnel.le.s de l’information, Laval, Québec.](https://cpi.fmdoc.org/wp-content/uploads/2024/11/La-gestion-des-donnees-de-recherche-briser-les-silos-et-batir-lavenir.pdf){target="\_blank"}
...
- [rsync](https://fr.wikipedia.org/wiki/Rsync){target="\_blank"}

- [SyncBack](https://www.2brightsparks.com/syncback/syncback-hub.html){target="\_blank"}

- [Duplicati](https://www.duplicati.com/){target="\_blank"}

- [Syncthing](https://syncthing.net/){target="\_blank"}

- [restic](https://restic.net/){target="\_blank"}
...
Many journals and [funding agencies](http://www.science.gc.ca/eic/site/063.nsf/fra/h_97610.html){target="\_blank"} now require to have archiving strategies. Why?
...
[The Directory of Open Access Journals](https://www.doaj.org/){target="\_blank"} is useful for searching for open access journals.
...
>
> [https://www.gbif.org/data-papers](https://www.gbif.org/data-papers){target="\_blank"}
...
url <- pins::board_url(
  list(
    seanoe = "https://www.seanoe.org/html/stat/2025/xzqksk6qr7-philippemassicottetakuvikulavalca.xls"
  )
)
...
df_viz <- df |>
  ...
  count(dataset_title, dataset_publication_year, country) |>
  mutate(
    dataset_title = str_wrap(
      glue::glue("{dataset_title} ({dataset_publication_year})"),
      40L
    )
  ) |>
...
df_viz |>
  ...
  labs(
    ...
    caption = "Cumulative downloads from year of publication through 2025"
  ) +
  ...
  theme(
    ...
    plot.caption = element_text(size = 8L, color = "white")
  )
```

## Open data repositories

- Scholars Portal Dataverse [https://dataverse.scholarsportal.info/](https://dataverse.scholarsportal.info/){target="\_blank"}
- Federated Research Data Repository [https://www.frdr-dfdr.ca/repo/?locale=fr](https://www.frdr-dfdr.ca/repo/?locale=fr){target="\_blank"}
- National Terrestrial Ecosystem Monitoring System (NTEMS) [https://opendata.nfis.org/mapserver/nfis-change_eng.html](https://opendata.nfis.org/mapserver/nfis-change_eng.html){target="\_blank"}
- Dryad [https://datadryad.org](https://datadryad.org){target="\_blank"}
- Catalogue de données de l'OGSL [https://ogsl.ca/fr/](https://ogsl.ca/fr/){target="\_blank"}
- Zenodo [https://zenodo.org/](https://zenodo.org/){target="\_blank"}
- Figshare [https://figshare.com/](https://figshare.com/){target="\_blank"}
- The Dataverse Project [https://dataverse.org/](https://dataverse.org/){target="\_blank"}
...
- [Back up your data externally as often as possible.]{.emphasize}
